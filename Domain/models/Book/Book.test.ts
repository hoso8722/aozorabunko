...
class MockBookRepository implements IBookRepository {
  public save = jest.fn() as jest.MockedFunction<(book: Book) => Promise<void>>;
  public findById = jest.fn() as jest.MockedFunction<(id: string) => Promise<Book | null>>;
}
...
describe('Book', () => {
  describe('registerBook', () => {
    it('should save a book with valid data', async () => {
      ...
      expect(savedBook.tags?.map(tag => tag.value)).toEqual(['Tag1', 'Tag2']);
    });
  });
});
